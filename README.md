...
* A reusable permission level two `/bfs debug followme` stick for independently selected groups of mobs. Arrival keeps a mob selected and waiting. Release the use button and click that mob again to release only it. Status pages, targeted stop and group stop expose membership, while chat and action bar messages report each meaningful outcome. Controller compatibility and final client verification remain tracked in the [follow evidence](docs/verification/follow-groups-20260915.md).
