...
Before any capture has completed, diagnostic status has no output path. Both server and client initialization leave that value absent instead of parsing a placeholder as a filesystem path. Completed capture filenames are unchanged, and enabling diagnostics is not required for startup.
...
For client-only rendering, interpolation, controller, or resource questions, use the separate local command `/bfs debug client on`, `/bfs debug client status`, and `/bfs debug client off` on the affected laptop. It is local only. The server cannot start it or inspect its files. Render samples include the selected variant, base and glow mask resource IDs, SHA 256 identities, raw brightness, selected layer, alpha-background check, and resource reload generation. Full command syntax, limits, artifact binding, parser invocation, privacy rules, and recovery for incomplete captures are in [BFS debug diagnostics](docs/test/debug-diagnostics.md).
...
Packaged multiplayer verification uses an installed production Forge `47.2.0` dedicated server on `node-1` and an isolated matching Prism instance on the Linux laptop. The Gradle development tasks above are not substitutes for packaged handshake or release evidence. Launch the installed server with the selected Java 17 executable and `@libraries/net/minecraftforge/forge/1.20.1-47.2.0/unix_args.txt nogui`. Verify the exact candidate and dependency hashes on both sides before connecting over the existing authorized private endpoint. A matching displayed mod version alone is insufficient. Keep online mode enabled and use the owned server console for fixture setup and cleanup.

DEC-012 defers laptop verification and performance comparisons for the current testing JAR endpoint. The preceding multiplayer procedure and [packaged performance procedure](docs/test/packaged-performance.md) remain available for a later requested pass. This disposition does not change earlier failed measurements or imply new client acceptance. The [installed compatibility fixture](docs/test/packaged-compatibility.md) exercises the pinned external mob on a production dedicated server without a client. Source binding and server gameplay checks remain mandatory.

Never run a windowed client, Xvfb, or rendering workload on `node-1`. Verify the laptop's desktop session and discrete GPU before a client launch. Set the isolated instance's master volume to zero before startup, then identify and mute only its exact application playback stream and verify mute state before acceptance. Stop the owned client if stream identity or mute state cannot be verified. For a hidden laptop capture, keep the Minecraft window on a nonactive Hyprland workspace and set `pauseOnLostFocus:false`. Resolve the exact owned Minecraft window through `hyprctl clients -j`, recording its PID, `stableId`, workspace, class, and title. On the current Hyprland session, `grim -T <stableId> <capture.png>` captures that foreign toplevel directly without switching workspaces. Accept it only when the target window row and active workspace match before and after capture, and retain the capture hash with those checks. `DISPLAY=:1 import -window <minecraft-window-id>` captures the active desktop instead of an inactive Xwayland workspace, so it is not admissible evidence. A portal capture requires explicit human selection; do not automate that selection.
