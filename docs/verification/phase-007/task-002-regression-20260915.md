...
The follow feedback oracle is also present in the repaired source. `BfsFollowManager.notifyOwner` sends each meaningful result to both the durable system chat and the action bar. The follow fixtures retain twenty members after arrival, release only the clicked member, and exercise the scheduler past the former 32 member boundary with 321 selected mobs. The waiting message identifies the mob, reports the remaining selected count, and says that moving farther away is unnecessary.
