...
The marker carries a versioned namespaced issue id and owner id. The server accepts it only when the current session issued that marker to the same owner. Restart, logout, death, respawn, and dimension changes clear issuance or leases. The marker is never consumed, repeated clicks on the already claimed target are idempotent, and a new issue does not silently transfer an existing lease.

Rejected clicks now report the reason to the owner in chat. A permission message means the player is not currently op, while an invalid or stale marker means `/bfs debug followme` must issue a fresh stick during the current server session.

If a mob only turns to face the owner and does not move, no follow lease is active. Have an operator keep the owner at permission level two, issue a fresh marker, and check `/bfs debug followme status <owner>` after one right click. The expected result is `permitted=true, issued=true, following=true`. `permitted=false` identifies the permission gate, `issued=false` identifies a missing or reset marker, and `following=false` means the click was rejected or the lease was released before movement began; use the rejection message and the follow capture records to identify the reason.

The harness supports every `Mob` family without a BFS or vanilla namespace whitelist, including bosses and external mod controllers. Players and nonmob entities are rejected. One owner can hold one lease, one mob can have one owner, and the server caps active leases at 32. A lease uses normal mob navigation or a SmartBrainLib `WALK_TARGET` adapter every 10 ticks, accepts a four block arrival radius, rejects targets beyond 64 blocks, expires after 2,400 ticks, and releases after 200 blocked ticks. It never teleports the mob or writes velocity directly. Explicit stop and lifecycle callbacks stop only the intent owned by the lease so ordinary goals can resume.

Follow captures use the existing `bfs-debug-v2` writer and the `follow` category. Events are `follow.claim`, `follow.reselect`, `follow.adapter`, `follow.intent`, `follow.progress`, `follow.block`, `follow.reject`, `follow.release`, and `follow.restore`. Owners and targets are session pseudonyms, entity types and reason codes; raw player UUIDs, entity UUIDs, marker NBT and issue tokens are omitted.
