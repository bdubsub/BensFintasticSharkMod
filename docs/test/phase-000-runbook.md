...
The latest verified run used phase revision `725bab666d38cd3f05e8e618f02fc96a6b84d7c7`, node 1 world `phase000-mp-final-2`, server port `25578`, RCON port `25579`, and the laptop endpoint `100.76.164.109:25578`. The laptop profile set `pauseOnLostFocus:false`, kept the client on workspace `3`, and left workspace `2` active. `DISPLAY=:1 import -window <minecraft_window_id> <capture_path>` collected the connected world and RCON-controlled state frames without changing the active workspace. The server marker `say phase 000 multiplayer animation capture complete` arrived in the laptop chat. The run is transport and controlled-state evidence; it is not a substitute for the full interactive animation, combat, cleanup, or population matrices.
