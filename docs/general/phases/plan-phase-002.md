...
This phase delivers the server owned temporary follow lease required for a marked stick to direct an independently selected group of eligible living mobs safely, retain selection while waiting or paused, and restore each released mob's ordinary controller without collateral behavior changes. It owns detailed execution of BFS2-REQ-008 and BFS2-REQ-009 only. The master remains authoritative for product scope, frozen IFC signatures, hosts, integration, and final endpoint.
...
**Objective:** Maintain independently toggled follow groups across every required controller family, preserve selection through arrival and recoverable pauses, provide truthful chat and action bar feedback, and restore ordinary behavior on explicit release or required lifecycle cleanup.  
**Owner:** Repository maintainer  
**Dependencies:** BFS2-PHASE-001, EXT-002  
...
- BFS2-REQ-008 creates the registry driven compatibility inventory, a server issued stick, acquisition validation, independent per operator groups with one owner per mob and no arbitrary selected follower cap, controller adapters, and real click paths.
...
- Laptop evidence proves actual right click input, joined correct dedicated server world, independent group selection, safe moving following, waiting and resume, deliberate per mob release, and matching chat and action bar feedback. Server evidence alone cannot close this residual client claim.
...
- Twenty and at least 33 independently selected mobs, deliberate toggles, arrival/resume, more than 2400 ticks of retention, temporary unheld stick, range and blocked pauses, targeted/group stop, security and lifecycle cleanup meet IFC-004 version 2. Held button and duplicate callbacks never toggle twice. Both user feedback channels agree with actual state and counts. Release affects only the intended members and ordinary behavior resumes within 40 ticks.
...
| IFC-003 | BFS2-PHASE-001 | Follow priority is below safety and above ordinary behavior, with safe pose, velocity, clearance, and reason. | Exercise every adapter against safe and blocked routes. | Pause on recoverable safety conditions; release on unrecoverable `ADAPTER_CONFLICT`. Never bypass safety. |
...
| EXT-001, deferred under DEC-012 | Existing laptop and private controls | Discrete renderer, desktop, private route, candidate identity, stream correlation, and mute can be revalidated. | Perform the host checks before client launch. | Keep only client gate unverified. Do not use node-1 graphical fallback. |
...
|---|---|---|---|---|
| IFC-004 | BFS2-PHASE-003 and BFS2-PHASE-007 | Version two lease state has server nonce, owner, mob, dimension, adapter, state, pause reason, progress ticks and owned intent, indexed by independent group membership. | Preserve the version 2 signature, reject invalid authorization and never restore live session leases from saved data. | Acquisition, release, restart, and parser packet. |
| Follow diagnostic vocabulary | Operator, support guide, final verification | `follow` category records acquisition, group revision/counts, press/outcome correlation, state, adapter, progress, scheduling delay, pause, resume, release and rejection without private names, chat text or token values. | Additive `bfs-debug-v2` typed payloads only. | Parser fixtures and sanitized capture. |
...
IFC-004 is produced here. The real interaction path supplies one physical press identity to server `claim`; issuance registry, held stack, permission and reach validation precede selection or release. Both callbacks and hands for that press reuse its outcome silently. A fresh press toggles only that mob; unrelated item interactions return `IgnoredUnrelated`. A lease owns one `follow` intent that feeds IFC-003 rather than setting velocity, clearing goal lists, teleporting, changing physiology, or moving across dimensions. `tick` validates owner, target, authorization and lifecycle before classifying following, waiting or paused. Distance, hand and recoverable path conditions pause only movement; elapsed selection age never expires membership. Route scheduling follows IFC-004 independently of group size. `release` removes only records identified by the lease token and restores the controller's own scheduling path.
...
|---|---|---|---|---|---|---|
| P002-TASK-001 | BFS2-REQ-008 | Before final adapters, add prototype `follow.claim`, `follow.adapter`, `follow.intent`, `follow.progress`, `follow.reject`, `follow.release`, and `follow.restore` records to the existing capture path, then create the registry compatibility inventory and acquire short lived prototype leases. Use named fixtures: `minecraft:zombie` on a 24 by 4 block solid floor with an L shaped two block wide obstacle route, `minecraft:salmon` in a 24 by 4 by 4 source water channel, `minecraft:phantom` in a 32 by 16 by 16 unobstructed air volume, `minecraft:frog` across a 12 block shallow source water and 12 block solid land route, `minecraft:villager` on a 24 by 4 solid route, `bensfintasticsharks:harbor_seal` in a 24 by 4 by 4 source water channel, `minecraft:ender_dragon` through a clicked dragon part in a 64 by 32 by 64 clear End arena, and `minecraft:wither` in a 48 by 24 by 48 clear arena. Each positive fixture must emit claim and adapter records in the acquiring server tick, record at least one safe ten tick route update and reduced owner distance by tick 400, then release and prove an ordinary valid controller decision by tick 40. A blocked fixture closes that route with an unpassable barrier, reaches a stable retained pause at 200 blocked ticks, then reopens it and requires resumed safe route progress without reclaiming within 400 ticks. Explicit release afterward proves ordinary selection within 40 ticks. Before a real external artifact is loaded, the mandatory preflight selects an official redistributable Forge 47.2.0 compatible source and records exact artifact URL, version, SHA 256, SHA 512, license, security inspection, entity ID, and controller path. Then exercise that non BFS entity on a 24 by 4 obstacle route. | BFS2-PHASE-001, EXT-001, approved source and artifact provenance, IFC-001, IFC-003. | Versioned manifest, pinned artifact preflight, prototype acquisition captures, and adapter decision record. | Existing capture components, controller adapter boundary, IFC-004, IFC-003, registry capability detection. | Manifest contains every active family and named fixture. Each mandatory case has its observable prototype acquisition, positive route, negative safety route, exact deadline, restoration oracle, and recoverable conflict result. |
| P002-TASK-002 | BFS2-REQ-008 | Implement `/bfs debug followme` issuance, persistent server session issuance authorization, namespaced marker version and issue identifier bound to owner, both hand callback deduplication, claim arbitration, and every adapter recorded in task 001. A successful `claim` generates a unique server only per lease nonce and stores it only in the IFC-004 lease record. Release invalidates that lease nonce and record but leaves a valid held issued stick reusable during the same server session. Restart clears the issuance registry and all lease records, so prior markers are denied until reissued. Implement the IFC-004 four block arrival band plus body dimensions, 0.5 block resume hysteresis, ten tick minimum route interval and fair 32 evaluations per tick scheduler. Replace the single owner entry with independently indexed members. Remove the automatic selection lifetime, global membership cap and arrival latch. Keep a separate short lived movement intent expiry. Retain selection in recoverable pauses and expose paginated group status and stopone plus group stop. Implement localized chat and action bar outcome delivery with meaningful transition batching. Reissuing a stick preserves valid group membership while rotating issuance. | P002-TASK-001, IFC-001, IFC-002, IFC-003. | One reusable legitimate issued stick, independent group membership and one truthful toggle outcome per deliberate click. | Existing command and debug components, Forge interaction callbacks, controller adapters, IFC-004. | Real click and server callback tests prove independent 20 and 33 member groups, one writer per mob, repeated deliberate toggles beside the owner, no double mutation or false feedback, target part resolution, safe obstacle following, owner conflict, invalid target and no velocity bypass. Use the group and feedback oracle below. |
| P002-TASK-003 | BFS2-REQ-009 | Implement idempotent release and lifecycle observers. Release within one server tick for deliberate same target click, stopone, group stop, invalid or revoked issuance, permission loss, target or owner death/removal, logout, dimension change, unload, shutdown and unrecoverable adapter conflict. Target events affect one member; owner events affect only that owner group. Arrival, age, selecting another mob, temporary unheld stick, range excess and blocked route retain membership through waiting or pause. Recheck blocked routes at most every 20 ticks and resume automatically when safe. Invalidate nonce at restart. Send truthful connected-recipient feedback and a bounded same-session rejoin cleanup summary. | P002-TASK-002, IFC-004. | Token scoped cleanup and restoration result for every exit. | Lease storage, lifecycle callbacks, adapter restoration boundary, IFC-004. | Each reason clears only owned intent, goal, modifier, and memory. Fixture ordinary valid goal selection resumes within 40 ticks with unchanged health and unrelated memory. |
| P002-TASK-004 | BFS2-REQ-008, BFS2-REQ-009 | Add follow category payloads and parser expectations before broad real path testing. Run bounded server and GameTest cases, then exact laptop residual input and moving owner test against a disposable dedicated server. | P002-TASK-002, P002-TASK-003, IFC-001, IFC-008, EXT-001. | Sanitized complete proof packets and an explicit result for every case. | BfsDebugManager, parser, support guide, GameTests, dedicated server fixture, local client debug capture. Apply DEC-012: performance and laptop portions are deferred; all other checks remain required. | All acceptance cases, negative security cases, retention, recovery, fair scheduling and feedback are correlated. Client evidence proves actual deliberate and held click input, world synchronization and readable chat/action bar results. Diagnostics off must preserve user feedback. |
...
### Group and feedback oracle

P002-TASK-002 implements this contract; P002-TASK-004 owns proof. Add fixtures through the actual server interaction entry path, with explicit press and callback correlation. First select twenty cows on a 48 by 24 solid arena with an eight block wide obstacle corridor and enough separate waiting space. Confirm every unique member remains in both indexes with one controller owner, selectedCount equals 20 and no prior member is released as the next is added. Guide the owner through the obstacle, wait nearby and move away in three cycles. In a clear reachable lane each mob must reduce owner distance by tick 400 and resume within its IFC-004 scheduling deadline after the owner leaves the arrival band. Membership remains twenty throughout; collision may limit individual velocity but never erase selection. A fresh same-position click releases just the clicked member, returns count 19 and restores its ordinary goal within 40 ticks. A following fresh click selects it again without requiring distance. `stopone` and idempotent group `stop` produce correct affected/remaining counts. Two operators each own independent groups; claiming an owned member rejects without altering either group.

Run a separate 33 member fixture to cross the old server cap, then a bounded scheduler test at 321 eligible entries to cross one ten tick scheduling window. The latter proves fair scheduling and count invariants, not real mob locomotion. Independently count actual route calls per tick and maximum time since each eligible member was serviced. Never derive the expectation by calling the production scheduler. A sustained group retains all members for 2401 ticks, including a waiting period, a temporarily unheld legitimate stick, a loaded mob outside the 64 block movement range, and a closed then reopened route. At 200 blocked ticks it remains selected and reports a stable pause. Restoring the condition resumes without a new claim; route progress in the reachable fixture must recover by tick 400. Capture groups of at most 32 in separate cohorts; a constant size witness tracks total membership, illegal release count, route evaluations, starvation and first failure over all ticks. Captures and witness must be complete. Diagnostic target limits cannot become gameplay caps.

Physical input tests use both hands and both Forge interaction callbacks for the same press, callbacks spanning adjacent ticks, holding use for 100 ticks, two deliberate presses without movement, rapid different-target clicks, and click plus removal/permission loss in the same tick. Require one mutation per real press, no toggle for held repeats or duplicate delivery, no success from an unauthorized active-lease shortcut, no false rejection after accepted selection/release, and no ordinary feeding/taming/advancement side effect for a consumed marked-stick action. Empty offhand and unrelated item actions produce no follow feedback. Reissuing the stick keeps membership and invalidates old authorization. Controller family fixtures include cow, slime and polar bear as regressions for reported no movement and accidental ordinary interaction, alongside the existing named Brain, aquatic, flying, boss and pinned third party cases.

Observe every issuance, selected, waiting, resumed, paused with reason, rejected, released and group cleared outcome through its real path. Server tests inspect the emitted localized components and per-channel recipient, counts, ordering and deduplication against independently observed state. One explicit action emits one chat receipt and one action bar receipt in the same processing tick. State transitions emit once, with at most ten ticks for grouped transitions. A stable waiting/blocked period emits no repeated transition messages. Chat preserves all meaningful transition kinds and affected members even when action bar text is superseded. An offline cleanup has no attempted live delivery and yields one bounded summary at next same-session login. Verify status pagination across twenty and 33 entries. Repeat with verbose capture off and on to prove feedback remains available and diagnostic state does not change behavior.

Laptop acceptance uses the exact matched candidate and owned dedicated server after server proof. Use actual clicks for twenty group selections, a 100 tick held input, arrival/resume, one deliberate release/reselect beside the owner, and group stop. Capture readable chat history and action bar outcomes with correct target labels/counts; verify pause and genuine rejection wording in separate short cases. No generic could-not-claim message or move-farther-before-selecting instruction may appear for a successful action. Preserve only sanitized evidence and clean the exact fixture, capture, world and process resources after their final consumer. A failed input or feedback gate keeps this phase open.
...
An operator owns an indexed collection of independent leases; each mob has at most one owner. Validate a new member fully without changing existing members. A new deliberate click on an already selected mob releases only it, even in the arrival band. Another operator receives `OWNER_CONFLICT`, never a steal. Keep group counts and revision consistent with the target index through every partial failure and lifecycle callback. The lease stores UUIDs and dimension IDs rather than live entity references, has no persistent follow flag, and dies at restart. Issuance is separate from membership. A selected lease is following, waiting or paused; deliberate release or required lifecycle cleanup terminates it. Invalid and unrelated interactions never create a member. It is serialized on the server thread. `release` is safe to repeat after any partial callback sequence and invalidates only the per lease nonce before adapter cleanup.

Every follow route creates an IFC-003 `follow` intent with current settings revision and a short expiration independent of the nonexpiring selection. Arrival retains follow priority. A recoverable pause clears powered motion while allowing survival recovery and suppressing competing combat/boat pursuit. No paused mob is teleported, force loaded, moved to a fixture or deleted. All routes share the bounded fair IFC-004 scheduler; delayed work never evicts a member. The destination follows the owner without collision, drowning, surface, or controller bypass: aquatic entities retain wet safe body envelopes, ground entities retain traversable terrain, flying entities retain bounded safe air path, and no route crosses dimension boundaries. Safety and survival outrank follow. Combat and ordinary state resume only through their normal validity checks after token scoped cleanup. Actual displacement, blocked routes, and solver reductions are recorded separately from desired follow target.
...
| P002-RISK-002. Forged, copied, stale or repeated interaction. | Marker, permission, target reach, press identity and cached outcome disagree. | Reject genuine invalid attempts without mutation; consume duplicates silently, leave unrelated items and empty offhand callbacks alone. Active membership never bypasses validation. | Keep legitimate marker reusable; expire bounded deduplication state at lifecycle exit. | Forgery, copy, stale restart, denied selected target, both callbacks/hands, held button and fresh press fixtures. |
| P002-RISK-003. New selection replaces existing members, arrival clears selection, or a toggle runs twice. | Group and target indexes, revision/count, state and per press outcomes disagree. | Add independently, retain waiting membership, toggle only the clicked mob once; reject theft without mutation. | Repair only the affected membership and preserve other members. | 20 and 33 members, arrival/resume, deliberate same spot toggles, held input, two operators and targeted stop. |
| P002-RISK-004. Safety route fails, exceeds range or makes no progress. | Clearance, distance, progress, blocked ticks and pause reason. | Hold safely and retain selection. After 200 blocked ticks keep a stable pause with bounded retries. No automatic timeout or relocation. | Resume when safe without another claim. Explicit release restores ordinary behavior within 40 ticks. | Block/reopen, leave/return range, temporary unheld stick and 2401 tick retention fixtures. |
...
**Controls:** Reuse permission level two `/bfs debug on follow <ticks> [targets]`, `/bfs debug status`, and `/bfs debug off` from IFC-001. The local console syntax is `/bfs debug followme <recipient>`, `/bfs debug followme status <recipient> [page]`, `/bfs debug followme stopone <target> <recipient>`, and `/bfs debug followme stop <recipient>`. Status lists all selected members through ten entry pages, with total and per state counts. Follow outcomes also produce chat and action bar feedback while capture is off. The scoped capture example is `/bfs debug on follow 400 <target>`. Server controls reject unprivileged sources. Console captures entity categories only with explicit targets. `status` must reveal side, target count, tick and wall deadline, counters, and exact output path.  
...
3. Issue a legitimate marker with `/bfs debug followme <recipient>`, inspect `/bfs debug followme status <recipient>`, use the real interaction entry path, move the target owner through the named fixture route, then trigger one required release stimulus with the relevant natural event or `/bfs debug followme stop <recipient>`. Correlate claim, adapter selection, intent, safety decision, route progress, release, controller restoration, footer, and parser result. Repeat separate bounded fixtures for each negative, retained pause, restart and absent target condition, plus the group and feedback oracle below. Verify a released legitimate marker can acquire a new lease before server restart.
...
|---|---|---|
| `follow.claim` | Server event, one per accepted interaction | One random per session lease alias, owner and mob pseudonyms, group revision and selected count, adapter, state and one owned intent. Selection has no expiry tick. |
...
| `follow.progress` | Server event, blocks and ticks | Progress refreshes under a valid route. At 200 blocked ticks membership remains in a stable paused state; reopening the route resumes progress without a new claim. |
...
| `follow.state`, `follow.group`, `follow.feedback` | Server transitions, revision, counts, outcome and channel codes | Independent members retain selection; waiting, paused and resumed differ from released. Emitted outcome and recipient agree with actual state. No raw chat is captured. |
| `follow.schedule` | Server count and ticks | At most 32 route evaluations per tick, no eligible starvation beyond IFC-004, no membership eviction. |
...
Document the marker issuance and interaction workflow, reusable issued stick versus per lease nonce boundary, eligible and ineligible target categories, independent groups without a count cap, one owner per mob, deliberate per mob toggle, arrival and pause retention without duration expiry, paginated status, targeted and group stop, both feedback channels, range and safety behavior, and the fact that third party support is adapter and artifact evidenced rather than a future universal promise. The support guide must contain the bounded follow capture procedure, exact console syntax, interpretation of `ADAPTER_CONFLICT`, `OWNER_CONFLICT`, and release reasons, redaction instructions, and escalation packet contents. Update documentation indexes and links. Associate phase work with its milestone and pull request during execution. After the approved merge, verified default, and tag, synchronize the wiki from the merged documentation. Publish no release.
