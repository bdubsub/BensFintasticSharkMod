...
**Owner:** Repository maintainer  
**Dependencies:** BFS2-PHASE-003, BFS2-REQ-013, BFS2-REQ-022, EXT-002  
...
| laptop client capability | EXT-001, deferred under DEC-012 | Actual laptop anchor, desktop session, discrete renderer, disposable instance, window-to-stream binding, and private server reachability can be verified at use time. | Perform the host and audio sequence in the local runbook. | Stop the owned client and leave only the client gate unverified. No node-1 graphics or singleplayer fallback. |
